...
Route::group(['prefix'=>'buyer','middleware'=>['auth','inactiveuser','UserRoleCheck','userEmailVerify','setlang']],function(){
    ...
    $user = \App\User::find(1884); // your own ID
    $user->notify(new \App\Notifications\OrderNotification(123, 5, 1884, 1884, 'Test Live Notification'));
...
///
Route::get('/Custom-offer','Frontend\BuyerCustomController@index')->name('buyer.Custom');
Route::get('/accept-offer/{id}','Frontend\BuyerCustomController@acceptOffer')->name('buyer.job.accept');
Route::post('/accept-request', 'Frontend\BuyerCustomController@Accept_custom_offer')->name('buyer.job.request.accept.hired');
Route::get('/Decline-request/{id}', 'Frontend\BuyerCustomController@Decline_custom_offer')->name('buyer.decline.custom.offer');
Route::get('/Decline-laterequest/{id}', 'Frontend\BuyerCustomController@Decline_custom_lateoffer')->name('buyer.cancel.lateoffer');

Route::get('/Decline-pending-request/{id}', 'Frontend\BuyerCustomController@Decline_pending_lateoffer')->name('buyer.cancel.pending.lateoffer');


Route::get('/Accept-Time-Extension/{id}', 'Frontend\BuyerController@AcceptTimeExtension')->name('buyer.accept.timeextesion');

///enterprise

Route::get('/enterprise/register', 'Frontend\EnterpriseController@create')->name('buyer.enterprise.create');
Route::post('/enterprise/save', 'Frontend\EnterpriseController@store')->name('buyer.enterprise.store');
Route::get('/enterprise/edit/{id}', 'Frontend\EnterpriseController@edit')->name('buyer.enterprise.edit');
Route::post('/enterprise/update/{id}', 'Frontend\EnterpriseController@update')->name('buyer.enterprise.update');
    ...
    //loc
     Route::post('/update-location', 'Frontend\BuyerController@locationupdate')->name('location.update');
    Route::get('/show-location/{id}', 'Frontend\BuyerController@locationshow')->name('location.show');
     ...
     //parital
      Route::get('/parital-order-amount-request/{id}','Frontend\BuyerController@ParitalAmountRequestApprove')->name('buyer.accept.paritalamount');
       Route::get('/parital-order-amount-request-cancel/{id}','Frontend\BuyerController@ParitalAmountRequestCancel')->name('buyer.cancel.paritalamount');
    ...
    Route::get('/flutterwave-ipn/CompanyRequest','Frontend\ExtraServicePaymentController@CompanyRequestflutterwave_ipn')->name('buyer.CompanyRequest.payment.flutterwave.ipn');
    ...
    Route::get('notification/all-notifications','Frontend\BuyerController@allNotification')->name('buyer.notification.all');
      Route::get('buyer/general-notification/view/{id}','Frontend\BuyerController@generalpushnotificationview')->name('buyer.gernel.notifications');
