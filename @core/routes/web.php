...
Route::group(['middleware' => ['globalVariable', 'maintains_mode','setlang']], function () {
                ...
                Route::middleware('auth')->post('/services/{service}/bookmark-toggle','Frontend\BookmarkController@toggle')->name('services.bookmark.toggle');
                Route::middleware('auth')->group(function () {
                    Route::get('/bookmark', 'Frontend\BookmarkController@index')
                        ->name('bookmarks');
                });
            Route::get('/challenge', 'FrontendController@FetchProjects')->name('challenges');
            Route::post('/project/vote/{id}','VoteController@vote')->name('project.vote')->middleware('auth');
            Route::get('/projects/{project}/comments', 'CommentController@fetch');
            ...
            Route::post('/comments/{comment}/reply', 'CommentController@reply')->name('comments.reply');
                Route::get('challenge/{slug}', 'FrontendController@single')->name('seller.project.single');
                Route::delete('/comments/{comment}', 'CommentController@destroy')->name('comments.destroy');

       
    Route::get('/', 'FrontendController@index')->name('homepage');
    Route::get('/ticket', 'FrontendController@ticket')->name('ticket');
     Route::post('language-chnage', 'FrontendController@set_language')->name('frontend.langchange');
    ...
    //Route::get('/privacy-policy', 'FrontendController@home_search_two')->name('frontend.home.search.two');
    Route::get('/home-search/single-page', 'FrontendController@home_search_single_page')->name('frontend.home.search.single');
    
    
    Route::get('/live', 'FrontendController@live_stream')->name('live')->middleware('auth');
    Route::get('/agora/token', 'Frontend\AgoraController@generateToken')->middleware('auth');
    Route::get('/agora/rtm-token', 'Frontend\AgoraController@generateRtmToken');
...
//    Route::match(['get', 'post'], '/email-verify', 'Auth\RegisterController@emailVerify')->name('email.verify')->middleware('auth:web');
    ...
    Route::get('/embed/{slug}', 'Frontend\ServiceListController@embed');
    ...
    //protfolio
      Route::get('portfolio/{id}', 'Frontend\PortfolioController@singleportfolio')->name('seller.portfolio');
    //all category
    ...
    //testersms
         Route::get('sendtestsms/', 'Frontend\ServiceListController@testersms')->name('seller.sendtestsms');
...
// frontend ends
// Route::middleware('auth')->group(function () {
//     Route::post('/services/{service}/bookmark', 'Frontend\BookmarkController@store')
//         ->name('services.bookmark');

//     Route::delete('/services/{service}/bookmark', 'Frontend\BookmarkController@destroy')
//         ->name('services.bookmark.remove');
// });
...
Route::middleware(['setlang'])->group(function () {
    ...
    // OTP verify (Step-2)
Route::post('/login/admin/otp/verify', 'Auth\LoginController@verifyAdminOtp')->name('admin.login.otp.verify');

// OTP resend
Route::post('/login/admin/otp/resend', 'Auth\LoginController@resendAdminOtp')->name('admin.login.otp.resend');

});

// ClickPesa server-to-server webhook + shared browser-return handler
Route::match(['get','post'],'/clickpesa/webhook','ClickPesaWebhookController@handle')->name('clickpesa.webhook');
Route::match(['get','post'],'/clickpesa/return','ClickPesaWebhookController@returnFromCheckout')->name('clickpesa.return');
