...
@section('style')
    @include('frontend.pages.services.service-book-style')
    <style>
        .input-with-icon { position: relative; }
        .input-icon { position: absolute; top: 50%; right: 10px; transform: translateY(-50%); color: #999; }
        .paymentGateway_add__item { width: calc(100% / 5 - 16px); overflow: hidden; }
        @media screen and (max-width: 1199px) and (min-width: 992px) { .paymentGateway_add__item { width: calc(100% / 3 - 13.33px); } }
        @media only screen and (max-width: 991px) { .paymentGateway_add__item { width: calc(100% / 4 - 15px); } }
        @media only screen and (max-width: 767px) { .paymentGateway_add__item { width: calc(100% / 3 - 13.33px); } }
        @media only screen and (max-width: 425px) { .paymentGateway_add__item { width: calc(100% / 2 - 10px); } }
        .custom_radio__inline__two .custom_radio__single { width: calc(56% - 8px); }
    </style>
@endsection
                ...
                <form action="{{ route('service.create.order') }}" id="msform" class="msform ms-order-form" method="post" name="msOrderForm" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="row g-4 mt-1">
                        ...
                        <div class="col-xl-9 col-lg-8">
                                                    ...
                                                    <p class="new_serviceDetails__author__para">
                                                        @if(!empty(optional(optional($service_details_for_book)->seller)->username))
                                                            <a href="{{ route('about.seller.profile', optional(optional($service_details_for_book)->seller)->username) }}">
                                                                {{ optional($service_details_for_book)->seller->username }}</a>
                                                        @endif
                                                    </p>
                                                </div>
                                            </div>
                            ...
                            <fieldset class="padding-top-50 edit_style_service_info">
                                                ...
                                                <div class="new_packageBook__details__item">
                                                    ...
                                                    <div class="new_packageBook__header">
                                                        <div class="new_packageBook__header__left">
                                                            <h4 class="new_packageBook__details__title">{{ get_static_option('service_main_attribute_title') ?? __('Choose package') }}</h4>
                                                        </div>
                                                    </div>
                                                    <!-- Heading send -->
                                                    @if($service_details_for_book->is_service_online == 1)
                                                                        ...
                                                                        <div class="new_packageBook__addFeature__flex">
                                                                            ...
                                                                            @if($service_details_for_book->is_service_online !=1)
                                                                                <div class="btn-wrapper">
                                                                                    ...
                                                                                    {{-- Decrease button hidden: buyer can only increase quantity or Remove --}}
                                                                                    <span class="substract package_quantity__icon include_service_qty_decrement" style="display:none;">
                                                                                        <i class="fa-solid fa-minus"></i></span>
                                                                                        <input type="number" min="{{ $include->include_service_quantity }}"
                                                                                               class="quantity-input package_quantity__input inc_dec_include_service"
                                                                                               data-id="{{ $include->id }}"
                                                                                               data-price="{{ $include->include_service_price }}"
                                                                                               data-quantity ="{{ $include->include_service_quantity }}"
                                                                                               data-min="{{ $include->include_service_quantity }}"
                                                                                               value="{{ $include->include_service_quantity }}"
                                                                                               onkeydown="return false;"
                                                                                               onpaste="return false;"
                                                                                               oninput="validateNumberInput(this); enforceMinQty(this);">
                                ...
                                @if($service_benifits->count() >1)
                                    <div class="overview-single padding-top-60">
                                        <h4 class="title">{{ get_static_option('service_benifits_title') ?? __('FAQ about the service:') }}</h4>
                                        <ul class="new_packageBook__list list_none mt-4">
                            ...
                            {{-- Mobile-only coupon input (always visible; the sidebar coupon is far below on mobile) --}}
                            <div class="mobile-coupon-inline d-lg-none mt-4" style="display:block;">
                                <label class="mobile-coupon-inline__label" style="display:block;">{{__('Have a coupon?')}}</label>
                                <div class="mobile-coupon-inline__row" style="display:flex;gap:8px;align-items:stretch;width:100%;">
                                    <input type="text" name="mobile_coupon_code" class="coupon_code mobile-coupon-input" placeholder="{{__('Enter Coupon Code')}}" autocomplete="off" style="display:block !important; flex:1 1 auto !important; min-width:0 !important; width:100% !important; visibility:visible !important; opacity:1 !important; height:44px !important;">
                                    <button type="button" class="apply-coupon mobile-coupon-btn" style="flex:0 0 auto;">{{ __('Apply') }}</button>
                                </div>
                                <span class="coupon_amount_for_apply_code d-block mt-1"></span>
                            </div>
                        </div>
