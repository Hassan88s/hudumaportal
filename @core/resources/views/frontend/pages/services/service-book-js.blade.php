@section('scripts')
    <script>
        ...
        /* Always inject the current CSRF token on every AJAX request
           (fixes 419 on schedule/coupon AJAX when the cached app.js token becomes stale) */
    ...
    <script>
        (function($) {
            ...
            $(document).ready(async function() {
                ...
                //Service start — Remove one include (blocked if it's the last one)
                $(document).on('click', '.remove-service-list', function() {
                    let remainingInputs = $('input.inc_dec_include_service:visible').length;
                    if (remainingInputs <= 1) {
                        toastr.warning("{{ __('You must keep at least one package selected.') }}");
                        return;
                    }
                    let include_service_id = $(this).data('id');
                    // Remove BOTH the left "choose package" card and the right summary line
                    $('.include_service_id_' + include_service_id).remove();
                    recalcPackageFee();
                })

                // Guard: if buyer somehow lowers the qty (e.g. via native spinner
                // arrows in Firefox), snap it back up to the seller-defined minimum.
                window.enforceMinQty = function (el) {
                    let $el = jQuery(el);
                    let min = Number($el.attr('min')) || Number($el.data('min')) || 1;
                    let cur = Number($el.val()) || 0;
                    if (cur < min) $el.val(min);
                };

                /**
                 * Recalculate package fee from currently-visible include qty inputs.
                 * Deduped by data-id so no include is counted twice.
                 *
                 * Per-include math:
                 *   data-price    = TOTAL price for the INITIAL quantity (e.g. 3000 for 3 units)
                 *   data-quantity = the initial quantity              (e.g. 3)
                 *   unit_price    = data-price / data-quantity        (e.g. 1000)
                 *   line_total    = current qty * unit_price          (e.g. 4 * 1000 = 4000)
                 */
                function recalcPackageFee() {
                    let seen = {};
                    let include_total_price = 0;
                    $('input.inc_dec_include_service:visible').each(function () {
                        let id = String($(this).data('id'));
                        if (!id || seen[id]) return;   // skip dupes / missing ids
                        seen[id] = true;
                        let qty          = Number($(this).val()) || 0;
                        let totalForInit = Number($(this).data('price')) || 0;
                        let initQty      = Number($(this).data('quantity')) || 1;
                        if (initQty <= 0) initQty = 1;
                        let unitPrice    = totalForInit / initQty;
                        include_total_price += qty * unitPrice;
                    });
                    // Round to 2 decimals to avoid float noise like 4000.0000000001
                    include_total_price = Math.round(include_total_price * 100) / 100;
                    $('input[name="package_fee_input_hiddend_field_for_js_calculation"]').val(include_total_price);
                    $('.package-fee').text(site_default_currency_symbol + include_total_price);
                    subtotal_calculate();
                    total_amount();
                }
                 // Example: Get default quantity from a server-side script
            function getDefaultQuantity() {
              let quantity = $('.inc_dec_include_service').data('quantity');
               let main_price = $('.inc_dec_include_service').data('price');
            let number= main_price/quantity;
           // return Math.ceil(main_price/quantity);
           var roundedNumber = parseFloat(number.toFixed(2));
           return roundedNumber; 
        }
                //Increment Decrement include service (fires on +/- click AND on input keyup)
                //Runs on a small setTimeout so the theme's .plus stepUp() (which
                //increments the input value) has finished BEFORE we read the value.
                $(document).on('keyup click', '.inc_dec_include_service, .include_service_qty_decrement', function() {
                    let $trigger = $(this);
                    setTimeout(function () {
                        // Resolve which input this event relates to (input itself, plus icon, or minus icon)
                        let $input = null;
                        if ($trigger.is('input') && $trigger.hasClass('inc_dec_include_service')) {
                            $input = $trigger;
                        } else if ($trigger.prev('input.inc_dec_include_service').length) {
                            $input = $trigger.prev('input.inc_dec_include_service');
                        } else if ($trigger.next('input.inc_dec_include_service').length) {
                            $input = $trigger.next('input.inc_dec_include_service');
                        } else {
                            $input = $trigger.closest('.package_quantity').find('input.inc_dec_include_service').first();
                        }
                        if (!$input || !$input.length) return;

                        let include_service_id = $input.data('id');
                        let quantity = Number($input.val());
                        if (isNaN(quantity) || quantity < 1) { quantity = 1; $input.val(1); }
...
//                  $seller_country = \App\Country::findOrFail($seller_city->country_id);
               ...
               var isServiceOnline = {{ $service_details_for_book->is_service_all_cities }};
               var countryAndCity = user_city  + ', ' + user_country;
          //  alert(isServiceOnline);
                ...
                $('.service_wise_seller_country').val(countryAndCity);
                   // Add an event listener for when the input value changes
                    ...
                    if (isServiceOnline == 0) {
    // Service NOT available everywhere, lock to provider's location
    $('#user_address').val(countryAndCity).attr('readonly', true);
} else {
    // Service available anywhere, allow user to enter their address
    $('#user_address').attr('readonly', false).attr('placeholder', 'Enter Your Location');
}
                @endif
