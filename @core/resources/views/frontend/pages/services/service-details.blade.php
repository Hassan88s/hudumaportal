...
@media screen and (max-width : 906px){
 ...
 .div-only-mobile{
  display: block;
    ...
    @include('frontend.pages.services.service-details-style')
@endsection
                    ...
                    <div class="service-details-wrapper">
                        <div class="service-details-inner">
                            ...
                            <ul class="author-tag style-02 mt-4">
                                <li class="tag-list" style="position: relative;">
                                    ...
                                    <a href="{{ route('about.seller.profile',optional($service_details->seller)->username) }}">
                                        <div class="authors">
                                            ...
                                            <span class="author-title"> {{ optional($service_details->seller)->username }} </span>
                                             @if(optional($service_details->seller->sellerVerify)->status==1)
                                        ...
                                        @if(auth()->check() && auth()->user()->user_type == '1')
                                                <button
                                                    type="button"
                                                    class="btn bookmark-btn {{ $isBookmarked ? 'btn-warning' : 'btn-outline-primary' }}"
                                                    data-url="{{ route('services.bookmark.toggle', $service_details->id) }}"
                                                >
                            ...
                            <ul class="details-tabs tabs margin-top-55">
                                ...
                                <li class="list" data-tab="tab2">
                                    {{ get_static_option('service_details_about_seller_title') ?? __('About Freelancer') }}
                                </li>
                                <li class="list" data-tab="tab3">
                                    {{ get_static_option('service_details_review_title') ?? __('Review') }}
                                </li>
                                
                                <li class="list" data-tab="tab4">
                                    {{ __('Portfolio') }}
                                </li>
                            ...
                            <div class="tab-content another-tab-content active" id="tab1">
                                ...
                                @if($service_includes->count() > 0)
                                    <div class="overview-single style-02 padding-top-60">
                                        <h4 class="title"> {{ get_static_option('service_details_what_you_get') ?? __('Available Packages:') }} </h4>
                                ...
                                <!--    <div class="overview-single style-02 padding-top-60">-->
                                <!--        <h4 class="title">{{ get_static_option('service_details_benifits_title') ?? __('FAQ about the service:') }} </h4>-->
                                <!--        <ul class="overview-benefits margin-top-30">-->
                                        ...
                                        <div class="seller-details-box margin-top-40">
                                            <ul class="seller-box-list">
                                                <li class="box-list"> {{ __('From') }}
                                                    <strong>
                                                        {{ optional(optional($service_details->seller)->city)->service_city }}
                                                    </strong>
                                                </li>
                                                @if(!empty($seller_since))
                                                    <li class="box-list"> {{ __('Freelancer Since') }}
                                                        <strong>
                                            ...
                                            @foreach($service_reviews as $review)
                                                <div class="about-seller-flex-content style-02">
                                                    ...
                                                    <div class="about-seller-content">
                                                        ...
                                                        <h5 class="title"> <a href="javascript:void(0)">{{ $review->buyer->username }}</a> </h5>
                                                        </a>
                                                        <div class="about-seller-list">
                            ...
                            @if(!empty($buyer_order_services))
                              ...
                              <div class="tab-content another-tab-content pt-5" id="tab4">
    <div class="details-content-tab">
        <div class="about-review-tab">
            @php
                $values = \App\Portfolio::with('images')->where('freelancer_id', $service_details->seller->id)->get();
            @endphp

            @if($values->count() > 0)
                @php
                    $featured = $values->first(); // default featured project
                    $others = $values->skip(1);   // thumbnails
                @endphp

                <!-- Featured Project -->
                <a href="{{ route('seller.portfolio', $featured->id) }}" id="featured-link">
                    <div class="featured-project card mb-4 p-3" id="featured-project" 
                         data-id="{{ $featured->id }}"
                         data-name="{{ $featured->name }}"
                         data-description="{{ Str::limit(strip_tags($featured->description), 120) }}"
                         data-image="{{ asset('@core/public/' . $featured->images->first()->image) }}"
                         data-date="{{ \Carbon\Carbon::parse($featured->created_at)->format('F Y') }}"
                         data-cost="${{ $featured->project_cost  ?? '' }}"
                         data-duration="{{ $featured->timeline  ?? '' }}">
                        <div class="row">
                            <div class="col-md-6">
                                <img id="featured-img" src="{{ asset('@core/public/' . $featured->images->first()->image) }}" 
                                     class="img-fluid rounded" alt="Featured Project">
                            </div>
                            <div class="col-md-6 d-flex flex-column justify-content-center">
                                <p class="text-muted" id="featured-date">From: {{ \Carbon\Carbon::parse($featured->created_at)->format('F Y') }}</p>
                                <h4 id="featured-title">{{ $featured->name }}</h4>
                                <p id="featured-desc">{{ Str::limit(strip_tags($featured->description), 120) }}</p>

                                <!--<div class="mb-2">-->
                                <!--    <span class="badge bg-light text-dark">Website Design</span>-->
                                ...
                                <div class="d-flex justify-content-between">
                                    <div>
                                        ...
                                        <p class="fw-bold" id="featured-cost">${{ $featured->project_cost ?? '' }} </p>
                                    </div>
                                    <div>
                                        <small class="text-muted">Project duration</small>
                                        <p class="fw-bold" id="featured-duration">{{ $featured->timeline ?? '' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="other-projects d-flex flex-wrap gap-3">
                    @foreach($values as $service)
                        @php
                            $firstImage = $service->images->first();
                        @endphp
                        @if($firstImage)
                            <div class="card border-0 thumbnail-link" 
                                 style="width: 150px; cursor:pointer;"
                                 data-id="{{ $service->id }}"
                                 data-name="{{ $service->name }}"
                                 data-description="{{ Str::limit(strip_tags($service->description), 120) }}"
                                 data-image="{{ asset('@core/public/' . $firstImage->image) }}"
                                 data-date="{{ \Carbon\Carbon::parse($service->created_at)->format('F Y') }}"
                                 data-cost="${{ $service->project_cost ?? '' }}"
                                 data-duration="{{ $featured->timeline  ?? ''  }}">
                                <img src="{{ asset('@core/public/' . $firstImage->image) }}" 
                                     class="card-img-top rounded" alt="Portfolio Thumbnail">
                            </div>
                        @endif
                    @endforeach
                </div>
...
<!--end portfolio-->
                      ...
                      @php
            $value = \App\Adspace::where(['id' =>'3'])->first();
            ...
            @endif
                    @if($recent_services->count() > 0)
                        <div class="another-details-wrapper padding-top-100">
                            <div class="section-title-two">
                                <h3 class="title">{{ __('Recent Services You Visit') ?? __('Recent Services You Visit') }} </h3>
                    ...
                    @endif
                        ...
                        <div class="another-details-wrapper padding-top-100">
                            <div class="section-title-two">
                                <h3 class="title">{{ get_static_option('service_details_another_service_title') ?? __('Related Services') }}</h3>
                    ...
                    @endif
                     ...
                     @php
            $value = \App\Adspace::where(['id' =>'4'])->first();
                    ...
                    <div class="service-details-package">
                        <div class="single-packages">
                            <ul class="package-price">
                                <li> {{ __('Starting at') }}</li>
                                <li> 
                                @if(is_null($service_details->starting_price))
                                                         {{ amount_with_currency_symbol($service_details->price) }}
                                                            @else
                                                          {{ amount_with_currency_symbol($service_details->starting_price) }}
                                                            @endif
                            ...
                            <div class="details-available-price margin-top-20">
                                <span class="summery-title">
                                    @if($service_details->is_service_online != 1)
                                        ...
                                        @if($service_details->delivery_days != 0)
                                ...
                                @if(moduleExists("LiveChat"))
                                    @if(Auth::guard('web')->check())
                                        @if(Auth::guard('web')->user()->user_type == 1)
                                         ...
                                         @php
                                                            $user_image = get_attachment_image_by_id(optional($service_details->seller)->image);
                                                        @endphp
                                            <br><a class="cmn-btn btn-bg-1 chat-toggle open-button live-chat-button-class-for-style" data-id="{{ $service_details->seller_id }}" data-user="{{ optional($service_details->seller)->username }}" data-user_image="{{ !empty($user_image) ? $user_image["img_url"] : null }}"><i class="las la-comments"></i> {{ get_static_option('service_chat_title') ?? sprintf(__('Chat With ').'%s', optional($service_details->seller)->username) }} </a>
                                        @endif
                                    @endif
                                @endif
                                 @if(!Auth::guard('web')->check())
                                  <a class="cmn-btn btn-bg-1 d-block" href="/login"> {{ ('Login to Chat') }} </a>
                                   @endif
                            </div>
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.bookmark-btn', function () {
            ...
            $btn.prop('disabled', true); // avoid double-click
            ...
            $.post(url, {}, function (res) {
                if (res.bookmarked) {
                    ...
                    $btn
                        ...
                        .html('★ Remove Bookmark');
                } else {
                    // Now unbookmarked
                    $btn
                        .removeClass('btn-warning')
                        .addClass('btn-outline-primary')
                        .html('☆ Add to Bookmarks');
                }
            }).fail(function (xhr) {
...
document.addEventListener("DOMContentLoaded", function() {
    document.querySelectorAll(".thumbnail-link").forEach(item => {
        item.addEventListener("click", function() {
            ...
            document.getElementById("featured-link").href = "/portfolio/" + this.dataset.id;
            document.getElementById("featured-img").src = this.dataset.image;
            document.getElementById("featured-title").innerText = this.dataset.name;
            document.getElementById("featured-desc").innerText = this.dataset.description;
            document.getElementById("featured-date").innerText = "From: " + this.dataset.date;
            document.getElementById("featured-cost").innerText = this.dataset.cost;
            document.getElementById("featured-duration").innerText = this.dataset.duration;
        });
    });
});
</script>
