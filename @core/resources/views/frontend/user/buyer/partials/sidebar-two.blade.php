        ...
        <div class="dashboard__bottom mt-5">
            <ul class="dashboard__bottom__list dashboard-list">
              <li class="dashboard__bottom__list__item ">
                    <a href="{{ route('seller.dashboard') }}"><i class="las la-home"></i> {{ __('HomePage') }}</a>
                </li>
                ...
                <li class="dashboard__bottom__list__item @if(request()->is('buyer/champions*')) active @endif">
                ...
                <!--@endif-->
                 @php
                $buyerid= \Illuminate\Support\Facades\Auth::guard('web')->user()->id;
                             $Custom = \App\CustomOffer::where(['buyer_id'=> $buyerid,'status'=>0])->get();
                         
                           
                        @endphp
      <li class="dashboard__bottom__list__item @if(request()->is('buyer/Custom*')) active @endif">
                    <a href="{{ route('buyer.Custom') }}"> <i class="las la-briefcase"></i>{{ __('Custom Service')}} <span class="badge_notification"> {{ $Custom->count() }} </span></a>
                </li>
...
@php
    $buyer_all_job_count = \Modules\JobPost\Entities\BuyerJob::where('buyer_id', \Illuminate\Support\Facades\Auth::guard('web')->user()->id)
        ->where('is_job_on', '1')
        ->count();

    $buyerJobs = \Modules\JobPost\Entities\BuyerJob::where('buyer_id', \Illuminate\Support\Facades\Auth::guard('web')->user()->id)
        ->where('is_job_on', '1')
        ->get();

    $count = 0;

    foreach ($buyerJobs as $job) {
        // Count only if no hired job request exists for this job
        $is_hired = \Modules\JobPost\Entities\JobRequest::where('job_post_id', $job->id)
            ->where('is_hired', 1)
            ->exists();
...
@endphp
                    ...
                    <li class="dashboard__bottom__list__item @if(request()->is('buyer/job/request/all*')) active @endif ">
                         @php
                             ...
                             $buyer_all_job_request = \Modules\JobPost\Entities\JobRequest::whereIn('job_post_id', $buyer_job_id)->where('is_hired','0')->count();
                        @endphp
                ...
                <li class="dashboard__bottom__list__item @if(request()->is('buyer/account-settings*')) active @endif">
                 ...
                 <li class="dashboard__bottom__list__item @if(request()->is('buyer/enterprise/register')) active @endif">
                    <a href="{{ route('buyer.enterprise.create') }}"> <i class="las la-user-alt"></i> {{__('Request Company')}} </a>
                </li>
                
                  <li class="dashboard__bottom__list__item">
                    <a href="{{ route('bookmarks')}}"> <i class="las la-bookmark"></i> {{__('My Bookmarks' )}} </a>
                </li>
