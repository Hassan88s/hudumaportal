...
@section('content')
 ...
 <div class="dashboard__headerContents">
                   ...
                   <div class="dashboard__headerContents">
                    <div class="dashboard__headerContents__flex">
                        <div class="dashboard__headerContents__left">
                            <h4 class="dashboards-title"><strong>{{__('Edit Project')}}</strong></h4>
...
@endif
     ...
     <form action="{{ route('seller.project.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data" class="p-4 border rounded bg-light">
    @csrf
    ...
    <select class="form-control" name="cate_id" required>
        @foreach($categories as $category)
            <option value="{{ $category->id }}" 
                {{ $portfolio->cate_id == $category->id ? 'selected' : '' }}>
                {{ $category->name }}
            </option>
        @endforeach
    </select>
...
<script>
    ...
    function deletePortfolioImage(imageId) {
        ...
        fetch("{{ route('seller.project.image.delete', ':id') }}".replace(':id', imageId), {
            method: 'POST',
            headers: {
