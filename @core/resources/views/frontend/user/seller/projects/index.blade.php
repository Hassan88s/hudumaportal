    ...
    <i class="fa-solid fa-plus"></i> {{ __('Add Project') }}
      ...
      @endif
                   <div class="dashboard__headerContents">
                    <div class="dashboard__headerContents__flex">
                        <div class="dashboard__headerContents__left">
                            <h4 class="dashboards-title"><strong>{{__('All Projects')}}</strong></h4>
                            ...
                            <table>
                                ...
                                @foreach ($projects as $project)
                                    <tr>
                                        ...
                                        <td data-label="{{ __('Job Title') }}"> {{ Str::limit($project->name,50) }} 
                                        <td data-label="{{ __('Description') }}">  
                                       {!! $project->description !!}
</td  >
                                        
                                        <td>
                                           <a href="{{ route('seller.project.edit', $project->id) }}" class="btn btn-primary">{{ __('Edit')}}</a>
                                          <form action="{{ route('seller.project.delete', $project->id) }}" method="POST" onsubmit="return confirm('Are you sure?')" class="pt-2">
    @csrf
