...
<div class="inner-wrap-contents">
    <p class="wrap-para">
        {{ __('Hello, Order Created By:') }} {{ optional($order_details->buyer)->name }} <br>
        {{ __('Order has been created successfully at:') }}  
        {{ optional($order_details->created_at)->toFormattedDateString() }}, 
        {{ ucwords(str_replace("_", " ", $order_details->payment_gateway)) }}
    </p>
    

    <h4 class="earning-order-title">
        {{ __('Your Order ID') }}: #{{ $order_details->id }}<br>
        {{ __('Total Amount') }}: {{ float_amount_with_currency_symbol($order_details->total) }}<br>
        {{ __('Tax Amount') }}: {{ float_amount_with_currency_symbol($order_details->tax) }} <br><br>
     ...
     <p class="wrap-para">
        Habari, Agizo Limeundwa Na: {{ optional($order_details->buyer)->name }} <br>
        Agizo limeundwa kwa mafanikio mnamo: 
        {{ optional($order_details->created_at)->toFormattedDateString() }}, 
        {{ ucwords(str_replace("_", " ", $order_details->payment_gateway)) }}
    </p>
    
    <h4 class="earning-order-title">
        Kitambulisho cha Agizo Lako: #{{ $order_details->id }}<br>
        Jumla ya Kiasi: {{ float_amount_with_currency_symbol($order_details->total) }}<br>
        Kiasi cha Kodi: {{ float_amount_with_currency_symbol($order_details->tax) }} <br><br>

        @if($order_details->transaction_id !='')
            Kitambulisho Chako cha Muamala: {{ $order_details->transaction_id }} <br>
        @endif
    </h4>
...
@if($order_includes->count()>=1)
    <h3 class="earning-title">Agizo ni pamoja na Maelezo</h3>
    <table class="table table-bordered table-responsive" style="margin: 0 auto; border: 1px solid #ddd; border-collapse: collapse; width: 100%; margin-bottom: 30px;overflow-x: auto;">
        <thead>
        <tr class="table-row">
            <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;">Kichwa</th>
            @if($order_details->is_order_online !=1)
                <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;">Bei ya Kitengo</th>
                <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;">Kiasi</th>
                <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;" class="table-heading">Jumla</th>
            @endif
        </tr>
        </thead>
        <tbody>
...
@if($order_additionals->count()>=1)
    <h3 class="earning-title">Agiza Maelezo ya Ziada</h3>
    <table class="table table-bordered" style="margin: 0 auto; border: 1px solid #ddd; border-collapse: collapse; width: 100%; margin-bottom: 30px;">
        ...
        <tr>
            ...
            <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;">Bei ya Kitengo</th>
            <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;">Kiasi</th>
            <th style="background-color: #111d5c; color: #fff; padding: 10px; text-align: left;">Jumla</th>
        </tr>
        </thead>
        <tbody>
        ...
        @endforeach
        <tr>
            <td colspan="3" style="padding: 10px"><strong>Ada ya Huduma ya Ziada</strong></td>
...
@if($order_details->is_order_online !=1)
    <div class="earning-wrapper">
        <h3 class="earning-title">Maelezo ya Malipo</h3><hr>
        <p class="wrap-para"><strong>Jina</strong> {{ $order_details->name }}</p>
    ...
    <div class="earning-wrapper">
        <h3 class="earning-title">Maelezo ya Usafirishaji</h3><hr>
        <p class="wrap-para"><strong>Jina:</strong> {{ $order_details->name }}</p>
        <!--<p class="wrap-para"><strong>{{ __('Email:') }}</strong> {{ $order_details->email }}</p>-->
        <!--<p class="wrap-para"><strong>{{ __('Phone:') }}</strong> {{ $order_details->phone }}</p>-->
        <p class="wrap-para"><strong>Jiji:</strong> {{ optional($order_details->service_city)->service_city }}</p>
        <p class="wrap-para"><strong>Eneo:</strong> {{ optional($order_details->service_area)->service_area }}</p>
        <p class="wrap-para"><strong>Nchi:</strong> {{ optional($order_details->service_country)->country }}</p>
        <p class="wrap-para"><strong>Anwani:</strong> {{ $order_details->address }}</p>
        <p class="wrap-para"><strong>Tarehe:</strong> {{ __($order_details->date) }}</p>
        <p class="wrap-para"><strong>Ratiba:</strong> {{ __($order_details->schedule) }}</p>
        <p class="wrap-para"><strong>Tarehe ya Kuunda Agizo :</strong> {{ optional($order_details->created_at)->toFormattedDateString() }}</p>
    </div>
@endif