                                ...
                                <tbody>
                                    @foreach($orders as $data)
                                        <tr>
                                            ...
                                            <td>
                                                @if($data->status =='5')
                                                <form action="{{ route('admin.orders.cancel.accept', $data->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        <button type="submit" class="btn btn-success btn-sm mt-1">{{ __('Accept') }}</button>
                                    </form>
                            
                                    <form action="{{ route('admin.orders.cancel.reject', $data->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        <button type="submit" class="btn btn-danger btn-sm mt-1">{{ __('Reject') }}</button>
                                    </form>
                                    @endif
