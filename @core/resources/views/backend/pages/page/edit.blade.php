                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{route('admin.page.update',$page_post->id)}}" method="post" enctype="multipart/form-data">
                           ...
                           @php
    $titleT = $page_post->title_translations ?? [];
    $contentT = $page_post->page_content_translations ?? [];

    $titleEn = old('title_en', $titleT['en'] ?? $page_post->title);
    $titleSw = old('title_sw', $titleT['sw'] ?? '');

    $contentEn = old('page_content_en', $contentT['en'] ?? $page_post->page_content);
    $contentSw = old('page_content_sw', $contentT['sw'] ?? '');
@endphp

<ul class="nav nav-tabs" role="tablist">
    <li class="nav-item">
        <a class="nav-link active" data-toggle="tab" href="#tab-en" role="tab" aria-controls="tab-en" aria-selected="true">EN</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#tab-sw" role="tab" aria-controls="tab-sw" aria-selected="false">SW</a>
    </li>
</ul>
...
<div class="tab-content margin-top-40">
    ...
    <div class="tab-pane fade show active" id="tab-en" role="tabpanel">
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="title_en" value="{{ $titleEn }}" id="title_en">
        </div>
        ...
        <div class="form-group classic-editor-wrapper @if(!empty($page_post->page_builder_status)) d-none @endif ">
            ...
            <input type="hidden" name="page_content_en" value="{{ $contentEn }}">
            <div class="summernote summernote-en" data-content="{{ $contentEn }}"></div>
        </div>
    </div>

    {{-- SW --}}
    <div class="tab-pane fade" id="tab-sw" role="tabpanel">
        <div class="form-group">
            <label>{{__('Title (SW)')}}</label>
            <input type="text" class="form-control" name="title_sw" value="{{ $titleSw }}" id="title_sw">
        </div>

        <div class="form-group classic-editor-wrapper @if(!empty($page_post->page_builder_status)) d-none @endif ">
            <label>{{__('Content (SW)')}}</label>
            <input type="hidden" name="page_content_sw" value="{{ $contentSw }}">
            <div class="summernote summernote-sw" data-content="{{ $contentSw }}"></div>
        </div>
    </div>
...
@section('script')
    ...
    <script>
        ...
        function initSummernote(selector, hiddenName) {
    const $el = $(selector);
    const initial = $el.data('content') || '';

    $el.summernote({
        height: 300,
        callbacks: {
            onChange: function(contents) {
                $('input[name="'+hiddenName+'"]').val(contents);
            }
        }
    });

    $el.summernote('code', initial);
    $('input[name="'+hiddenName+'"]').val(initial);
}

$(document).ready(function () {
    initSummernote('.summernote-en', 'page_content_en');
    initSummernote('.summernote-sw', 'page_content_sw');
});
