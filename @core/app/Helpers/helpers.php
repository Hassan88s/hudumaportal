...
if (!function_exists('linkify_message')) {
    /**
     * Escape HTML and convert URLs in plain text to clickable anchor tags.
     * Safe to use with {!! !!} in blade because text is escaped first.
     */
    function linkify_message($text)
    {
        ...
        return preg_replace_callback($pattern, function($m) {
            $url = rtrim($m[1], '.,;:!?)');
            return '<a href="' . $url . '" target="_blank" rel="noopener noreferrer" style="color:#6366f1;text-decoration:underline;word-break:break-all">' . $url . '</a>';
        }, $escaped);
    }
}

if (!function_exists('notifySeller')) {
    function notifySeller($seller_id, $pushMessage, $smsMessage, $data = [])
    {
        $isOnline = \Illuminate\Support\Facades\Cache::has('user-is-online-' . $seller_id);

        $seller = \App\User::find($seller_id);
        ...
        if ($isOnline) {
            $seller->notify(new \App\Notifications\GeneralNotification($pushMessage, $seller_id, $data));
        } else {
            sendSMS($seller->phone, $smsMessage);
        }
    }
}
...
if (!function_exists('sendSMS')) {
    function sendSMS($receiverNumber, $message)
    {
        ...
        try {
            $from = env('NEXTSMS_SENDER', 'Hudumapotal'); // Fallback if not in .env
            $reference = uniqid();
            ...
            if ($error) {
                \Log::error("NextSMS Error: " . $error);
            } else {
                \Log::info("SMS sent to {$receiverNumber}: " . $response);
            }
...
if (!function_exists('getnoofhiring')) {
    function getnoofhiring($para)
    {
    $count_orders =   Order::where('job_post_id',$para)->count();
        return $count_orders;
   
    }
}
if (!function_exists('getEnterpriseNamewithAuth')) {
    function getEnterpriseNamewithAuth()
    {
        $user = Auth::guard('web')->user();
        if ($user && $user->is_company == '1') {
            $enterprise = Enterprise::where('user_id', $user->id)->first();
            return $enterprise ? $enterprise->name  : $user->username;
        }
        return $user ? $user->username : 'Guest';
    }
}
if (!function_exists('getEnterpriseNamewithoutAuth')) {
    function getEnterpriseNamewithoutAuth($name)
    {
        $user = $name;
       if ($user && $user->is_company == '1') {
            $enterprise = Enterprise::where('user_id', $user->id)->first();
...
function ratting_star($ratting){
    ...
    $string = "";
    if($ratting < 1.5){
        $string = "<i class='las la-star active'></i>";
    }elseif ($ratting >= 1.5 && $ratting < 2){
        $string  = "<i class='las la-star active'></i><i class='las la-star-half'></i>";
    }elseif ($ratting >= 2 && $ratting < 2.5){
        $string  = "<i class='las la-star active'></i><i class='las la-star active'></i>";
    }elseif ($ratting >= 2.5 && $ratting < 3){
        $string  = "<i class='las la-star active'></i><i class='las la-star active'></i><i class='las la-star active'></i>";
    }elseif ($ratting >= 3 && $ratting < 3.5){
        $string  = "<i class='las la-star active'></i><i class='las la-star active'></i><i class='las la-star active'></i><i class='las la-star-half'></i>";
    }elseif ($ratting >= 3.5 && $ratting < 4){
        $string  = "<i class='las la-star active'></i><i class='las la-star active'></i><i class='las la-star active'></i><i class='las la-star active'></i>";
...
function sendPushNotificationWithAlias($userId, $titleEn, $titleSw, $messageEn, $messageSw, $url = null)
{
    $content = [
        "app_id" => "c1ba95cf-fa2e-4b7d-8be3-de039b9cc7f0", // Replace with your OneSignal App ID
       'include_external_user_ids' => ['1885'],
        "headings" => [
            "en" => $titleEn,
            "sw" => $titleSw
        ],
...
curl_setopt_array($curl, [
    ...
    CURLOPT_HTTPHEADER => [
        'Authorization: Basic os_v2_app_yg5jlt72fzfx3c7d3ybzxhgh6bzmgdrwugreum4m6thyllqfvydvpc5nfswhirvsvgt7xkhzgv76d26fochl4tusxlte4fiemxijk7i', // <- Replace this with actual REST API Key
        "Accept: application/json",
        "Content-Type: application/json"
    ],
]);
