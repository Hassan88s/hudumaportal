...
class ServiceListTwo extends PageBuilderBase
{
    public function preview_image()
    {
        return "service/service_list_two.png";
    }
    ...
    public function frontend_render()
    {
        ...
        $service_quyery = Service::query()->where("services.status", 1);
        ...
        if(!empty(get_static_option("google_map_settings"))) {
            if (!empty(request()->get('all_button_filter_value'))) {
                $service_quyery->where('services.status', 1);
        ...
        if(!empty(get_static_option("google_map_settings"))){
             ...
             if (!empty(request()->get('autocomplete_address'))) {
 if (empty(request()->get('remotely_button_filter'))) {
    if (!empty(request()->get('latitude')) && !empty(request()->get('longitude'))) {
        // Calculate the radius in kilometers (adjust as needed)
        $distance_radius_km_get = request()->get('distance_kilometers_value');
        ...
        // Calculate distance directly in the WHERE clause
        $service_quyery->join('users', 'services.seller_id', '=', 'users.id')
            ->whereRaw(
                "(6371 * acos(
                    cos(radians(?)) * cos(radians(users.latitude)) * cos(radians(users.longitude) - radians(?)) +
                    sin(radians(?)) * sin(radians(users.latitude))
        ...
        if (!empty(request()->get('price_range_value'))) {
            ...
            // Add your code to filter jobs using $minPrice and $maxPrice here
            $service_quyery->where('services.price', '>=', $minPrice);
            $service_quyery->where('services.price', '<=', $maxPrice);
        ...
        if (!empty(request()->get("q"))) {
            $service_quyery
                ->where("services.title", "LIKE", "%" . request()->get("q") . "%")
                ->orWhere(
                    "description",
                    "LIKE",
                    "%" . request()->get("q") . "%"
...
//        }
        ...
        // var_dump(,$IDorDate);die;
        // $order_by2='featured';
        // $order_by3 = 'sold_count'; 
        // $order_by4 = 'view';
        ...
        //      ->OrderBy($order_by2,$IDorDate)
        //      ->OrderBy($order_by3,$IDorDate)
        //      ->OrderBy($order_by4,$IDorDate)
        //      ->get();
            
$order_by1 = 'services.featured';
$order_by2 = 'services.sold_count'; 
$order_by3 = 'services.view';

$all_services = $service_quyery
    ->leftJoin('seller_subscriptions', 'seller_subscriptions.seller_id', '=', 'services.seller_id')
    ->leftJoin('subscriptions', 'subscriptions.id', '=', 'seller_subscriptions.subscription_id')
    ->where('services.status', 1) 
    ->where('services.is_service_on', 1) 
    //->where('seller_subscriptions.status', 1) 
  // ->where('seller_subscriptions.status', 1) // Ensure active sellersubscriptions
    ->when(subscriptionModuleExistsAndEnable('Subscription'), function ($q) {
        $q->whereHas('seller_subscription');
    })
    ->orderByDesc($order_by1) 
    ->orderByRaw("FIELD(subscriptions.title, 'VIP', 'Premium plus', 'Premium PLAN', 'Professional Plan', 'Starter Plan', 'Basic (FREE)') ASC") 
    ->orderByDesc($order_by2)
    ->orderByDesc($order_by3) 
    ->select('services.*') 
    ->get();
       ...
       $total_service= $all_services->count();
        //static text helpers
        ...
        return $this->renderBlade('service.service-list-two',[
