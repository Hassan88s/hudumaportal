...
class PopularService extends \App\PageBuilder\PageBuilderBase
{
 ...
 $book_appoinment = __($settings['book_appointment']) ?? __('Book Now');

        //static text helpers
        $static_text = static_text();

        $services = Service::select('id','title','image','description','price','slug','starting_price','seller_id','featured','service_city_id','is_service_online')
        ...
        foreach ($services as $service){
            ...
            $book_now = route('service.list.book',$service->slug);   
             if(is_null($service->starting_price)){
                                                        ...
                                                        $price =   amount_with_currency_symbol($service->starting_price) ;
    }
            ...
            $seller_name =  optional($service->seller)->name;
             $username =  optional($service->seller)->username;
            $seller_service_location = sellerServiceLocation($service);
            $seller_verifiy =optional($service->seller->sellerVerify)->status;
            ...
            $verify='';
                   if($seller_verifiy=='1'){
                                          $verify.='<div data-toggle="tooltip" data-placement="top" title="{{__("This seller is verified by the site admin according his national id card.")}}">
...
$image_badge = \Modules\Subscription\Entities\Subscription::where(['id' =>$value->subscription_id])->first('image');
// $badge =  render_image_markup_by_attachment_id($image_badge);
$badge = render_image_markup_by_attachment_id($image_badge->image,'','','thumb');
    }
}
                                ...
                                <li class="tag-list">
                                    <a href="{$seller_profile}">
                                        <div class="authors">
                                            ...
                                            <span class="author-title"> {$username} </span>
                                            {$verify}
                                           <div class="thumb">
                                            {$badge}
                                             </div>
