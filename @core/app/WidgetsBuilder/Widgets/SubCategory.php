...
class SubCategory extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $subcategories = SubCategoryModel::whereHas('services')->where('status',1)->select('id','name','slug')->orderBy($order_by,$IDorDate)->take($items)->get();
        $route = route('service.list.subcategory');
        
        $subcategory_markup = '';

       foreach ($subcategories as $sub_cat){
       $sub_category_name = $sub_cat->name;
       $sub_category_slug = $sub_cat->slug;
       $subcategory_markup.= <<<CATEGORY
    <li class="list"><a href="{$route}/{$sub_category_slug}">{$sub_category_name}</a></li>
CATEGORY;
