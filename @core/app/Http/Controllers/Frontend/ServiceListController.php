...
{ 
    ...
    protected function cancel_page(){
    	if(!empty(session()->get('order_id'))){
           Order::find(session()->get('order_id'))->update(['status' => 4,'payment_status'=>'cancalled']);
        }
    ...
    public function serviceDetails($slug)
    {
      // var_dump($slug);die;
        ...
        //   $Related_service = Service::where('category_id',$service_details->category_id)->inRandomOrder()->take(2)->get();
    $Related_service = Service::where('category_id', $service_details->category_id)
    ...
    ->get();
        ...
        $seller_name = User::select('username')->where('id', $service_details->seller_id)->where('user_status', 1)->first();
        ...
        $images = Service::select('image_gallery')->where('id', $service_details->id)->first();
                ...
                if (Auth::guard('web')->check()) {
                $buyer_order_services = Order::select('service_id', 'buyer_id')
                ...
                // Store/update in DB
                RecentServiceVisit::updateOrCreate(
                    ['user_id' => $userId, 'service_id' => $service_details->id],
                    [] // updated_at auto-refresh
                );
            
                // Fetch last 3 for logged-in user
                $recent_services = Service::whereIn('id',
                    RecentServiceVisit::forUser($userId)
                ...
                // --- Guest handling with session ---
                $visited = session()->get('guest_recent_services', []);
            
               
                $visited = array_filter($visited, fn($id) => $id != $service_details->id);
            
                // Add current service at beginning
                array_unshift($visited, $service_details->id);
            
                // Keep only last 3
                $visited = array_slice($visited, 0, 3);
            
                // Save back to session
                session()->put('guest_recent_services', $visited);
            
                // Fetch recent services (excluding current)
                $recent_services = Service::whereIn('id', $visited)
                    ->where('id', '!=', $service_details->id)
                    ->get();
            }
            $isBookmarked = false;
     ...
     public function embed($slug)
    {
        ...
        $service_details = Service::with('seller')->where('id', $slug)->firstOrFail();
        return view('frontend.pages.services.embed',compact( 'service_details',));
    }
    ...
    public function scheduleByDay(Request $request)
    {
        if ($request->ajax()) {
            ...
            // Only match ENABLED days (status=1) — sellers can toggle a day off
            // in /seller/availability without losing their saved schedules.
    ...
    public function createOrder(Request $request)
    {
            ...
            if($request->is_service_online_ != 1) {
                foreach ($request->services as $key => $requested_service) {
                   ...
                   // $package_fee += $requested_service['quantity'] * $service->include_service_price;
                      $package_fee = $service->include_service_price;
       ...
       $sub_total =  $request->service_subtotal_input_hidden_field_for_js_calculation;
        ...
        $order_message = __('Huduma yako imehifadhiwa! / Your service has been booked!');
    ...
    // NEW: Referral Stage-2/3 via ReferralService (idempotent).
    if ($user) {
        ...
        $rs->onBuyerSecondOrderWithin60Days($user);
                        ...
                        try {
                            $mail_subject = get_static_option('new_order_email_subject') ?? __('New Order #'). ' / Agizo Jipya #';
                             $mail_subject_for_seller = 'Your Service Has Been Booked!/Huduma yako imehifadhiwa!';
                            
                            $message_for_buyer = get_static_option('new_order_buyer_message') ?? __('You have successfully placed an order #'). ' / Umeweka agizo kwa mafanikio #';
                                $message_for_seller_admin = get_static_option('new_order_admin_seller_message') ?? __('You have a new order #'). ' / Una agizo jipya #';
                             $message_for_seller_admin = str_replace(["@name","@service_id","@id"],[$seller->username,$get_service_id_from_last_order->service_id,$order_details->id],$message_for_seller_admin);
                             
                              $message_for_buyer = str_replace(["@id"],[$order_details->id],$message_for_buyer);
                          
                            Mail::to($order_details->email)->send(new OrderMail(strip_tags($mail_subject).$order_details->id,$order_details,$message_for_buyer));
                            Mail::to($seller->email)->send(new OrderMail(strip_tags($mail_subject_for_seller).$order_details->id,$order_details,$message_for_seller_admin));
                            Mail::to(get_static_option('site_global_email'))->send(new OrderMail(strip_tags($mail_subject_for_seller).$order_details->id,$order_details,$message_for_seller_admin));
                            
                            
                             // SMS Notifications
                          $order_id = $order_details->id;
                            $website_link = "https://hudumaportal.co.tz/";
                            $message_for_buyer = "You have successfully placed the order. Your order ID is $order_id. Visit our website at $website_link for more details. " . 
                     "Umefanikiwa kuweka oda. Kitambulisho chako cha oda ni $order_id. Tembelea tovuti yetu $website_link kwa maelezo zaidi.";
  
                      sendSMS($get_buyer->phone, strip_tags($message_for_buyer));
                       $message_for_seller_admin = "A new order has been placed. The order ID is $order_id. Please check your $website_link dashboard for more details. " . 
                                    "Oda mpya imewekwa. Kitambulisho cha oda ni $order_id. Tafadhali angalia dashibodi yako $website_link kwa maelezo zaidi.";
                        sendSMS($seller->phone, strip_tags($message_for_seller_admin));
                                } catch (\Exception $e) {
                ...
                $seller->notify(new OrderNotification($last_order_id,$request->service_id, $request->seller_id, $buyer_id,$order_message));
                        ...
                        'payment_gateway' => 'Pay for booking Service',
                        'payment_status' => 'complete',
                        'status' => 1,
                ...
                $messages = get_static_option('abuyer-Service-booking_message') ?? '';
                $messages = str_replace(["@name","@clientname","@orderid"],[$seller->username,$buyer_info->username,$last_order_id],$messages);
               
                Mail::to($buyer_info->email)->send(new BasicMail([
                    'subject' => get_static_option('buyer-Service-booking_subject') ??  __('Your service booking request has been sent'),
                    'message' => $messages ?? '',
                ]));
                ...
                notifySeller(
                    $seller_id,
                    "Booking yako Huduma Portal (Order ID: $last_order_id) inasubiri kupitishwa. Tutakujulisha ikithibitishwa. / Your Huduma Portal booking (Order ID: $last_order_id) is awaiting approval. We’ll notify you once confirmed.", //p
                    "Booking yako Huduma Portal (Order ID: $last_order_id) inasubiri kupitishwa. Tutakujulisha ikithibitishwa.", //sms
                    [
        ...
        if ($request->selected_payment_gateway === 'clickpesa') {
            try {
                ...
                $orderRef = \App\Services\ClickPesaService::makeOrderReference('order', $last_order_id);
    ...
    public function categoryServices($slug = null)
    {
        ...
        if(!empty(request()->get('sortby'))){
            ...
            if (request()->get('sortby') == 'highest_price') {
             ...
             $service_quyery->orderBy('featured', 'Desc');
        }
    ...
    {
        $all_featurd_service = Service::select('id','title','image','description','starting_price','price','slug','seller_id', 'service_city_id', 'is_service_online')
            ->with('reviews')
    ...
    {
        $all_popular_service = Service::select('id','title','image','description','starting_price','price','slug','seller_id','view','featured', 'service_city_id', 'is_service_online')
            ->with('reviews')
 ...
 public function sendSMS($receiverNumber, $message)
{
    ...
    //       $message_for_buyer = "You have successfully placed the order.";
  
    //           sendSMS('255710568818', strip_tags($message_for_buyer));
    // }
