...
class ExtraServicePaymentController extends Controller
{
    ...
    public function flutterwave_ipn(Request $request)
    {
        try{
            ...
            if (isset($payment_data['status']) && $payment_data['status'] === 'complete'){
                $this->update_database($payment_data['order_id'], $payment_data['transaction_id']);
        $id=session()->get('main_orders_id');
       $finalTotal= session()->get('order_total');
       $finalCommission= session()->get('order_Commission');
       $finalTax= session()->get('order_Tax');
     ...
     public function CompanyRequestflutterwave_ipn(Request $request)
    {
        ...
        try{
            ...
            $payment_data = $flutterwave->ipn_response();
              $paymentData = session()->get('CRequest_payment_details');
        ...
        return response()->json(['status' => 'failed', 'message' => 'No payment session found'], 400);
            ...
            if (isset($payment_data['status']) && $payment_data['status'] === 'complete'){
                var_dump($paymentData);
                 Enterprise::create([
    'user_id' => auth()->id(),
    'name' => $paymentData['enterprise_name'],
    'description' => $paymentData['enterprise_description'],
    'business_type' => $paymentData['business_type'],
    'industry' => $paymentData['industry'],
    'enterprise_email' => $paymentData['enterprise_email'],
    'phone_number' => $paymentData['phone_number'],
    'website' => $paymentData['website'],
    'office_address' => $paymentData['office_address'],
    'representative_name' => $paymentData['representative_name'],
    'representative_position' => $paymentData['representative_position'],
    'representative_email' => $paymentData['representative_email'],
    'representative_phone' => $paymentData['representative_phone'],
]);
    session()->forget('CRequest_payment_details');
     toastr_success(__('Your Request Created Successfully'));
            return redirect()->route('buyer.enterprise.create');
            }
