...
//         ]);
        ...
        $page = Page::find($id);
         ...
         // allback columns (English)
    $page->title = purify_html($request->title_en);
    $page->page_content = $request->page_content_en;

    //  json translations
    $page->title_translations = [
        'en' => purify_html($request->title_en),
        'sw' => purify_html($request->title_sw),
    ];

    $page->page_content_translations = [
        'en' => $request->page_content_en,
        'sw' => $request->page_content_sw,
    ];
            $page->title =  purify_html($request->title);
            $page->page_content =  $request->page_content;

//  slug based on EN title
    $slugBase = !empty($request->slug) ? $request->slug : Str::slug($request->title_en);

    //  unique slug check (exclude current page)
    $exists = Page::where('slug', $slugBase)->where('id','!=',$page->id)->exists();
    $page->slug = $exists ? $slugBase . '-' . $page->id : $slugBase;
