...
class OrdersController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if ($request->ajax()){
            $data = Order::select('*')-> where('status', '!=' , 4)->whereNot('payment_status', '')
            ->orderBy('id','desc')
            ->take(20)->get();
    ...
    public function cancelOrders(){
        $orders = Order::whereIn('status', [4, 5])->latest()->get();

        return view('backend.pages.orders.cancelled',compact('orders'));
    }
    
    
    ///reject and accept cancel request of order 


        public function acceptCancelRequest($id)
        {
            $order_Detials = Order::findOrFail($id);
        ...
        ////
             ...
             $seller = User::where('id',$order_Detials->seller_id)->first();
                 try {
                         $mail_subject = __('Request Accepted for Cancel Order #') . ' / ' . __('Ombi la kughairi agizo limekubaliwa #');
                            $message_for_buyer = __('Your request to cancel the order has been accepted.') . ' / ' . __('Ombi lako la kughairi agizo limekubaliwa.');
                            $message_for_seller_admin = __('The buyer’s request to cancel the order has been accepted by the admin.') . 
                            ' / ' . __('Ombi la mnunuzi la kughairi agizo limekubaliwa na msimamizi.');
           ...
           return redirect()->back()->with('success', 'Cancel request accepted. Order marked as cancelled.');
        }
        ...
        public function rejectCancelRequest($id)
        {
            ...
            $order_Detials->status = 1; 
           $order_Detials->payment_status='complete';
            $order_Detials->save();
            
            $seller = User::where('id',$order_Detials->seller_id)->first();
                 try {
                          $mail_subject = __('Request Rejected for Cancel Order #') . ' / ' . __('Ombi la kughairi agizo limekataliwa #');
                          $message_for_buyer = __('Your request to cancel the order has been rejected.') . ' / ' . __('Ombi lako la kughairi agizo limekataliwa.');
                            $message_for_seller_admin = __('The buyer’s request to cancel the order has been rejected by the admin.') . 
                            ' / ' . __('Ombi la mnunuzi la kughairi agizo limekataliwa na msimamizi.');
...
//            event(new SupportMessage($ticket_info));
    ...
    public function change_payment_status($id){
        ...
        $payment_status = Order::select('id','seller_id','payment_status','email','name','job_post_id','order_from_job','Custom_offer_id')->where('id',$id)->first();
        ...
        if($payment_status->payment_status=='pending'){
            $new_status = 'complete';
             if($payment_status->Custom_offer_id != NULL){
             $custom_id=$payment_status->Custom_offer_id;
        CustomOffer::where('id', $custom_id)->update([
            'status' =>"1",
        ]);
             }
        }else{
