...
class SellerController extends Controller
{
    ...
    public function cronjob(){
     $allpromtped=Featureservice::all();
     $today =$expire_date = Carbon::now();
     foreach($allpromtped as $row) { 
     if($today > $row->expire_date){
       $service_id= $row->service_id;
    Service::where('id', $service_id)->update([
               'featured' =>"0"
                  ]);
    Featureservice::where('id',$row->id)->delete();              
     }
     }
    ...
    ///expire promoted with subscribtion
   public function expirepromoted()
{
    SellerSubscription::where('expire_date', '<', Carbon::now())
        ->where('status', '1')
        ->chunk(100, function ($sellerSubscriptions) { 
            foreach ($sellerSubscriptions as $row) {
                Service::where('seller_id', $row->seller_id)
                    ->where('featured', '1')
                    ->where('is_paid_promoted', '0')
                    ->update(['featured' => '0']); 
            }
        });
}
    ...
    ///time
     ///cron_job
        ...
        public function checkUpcomingOrderDate(){
        ...
        //   echo $date;
        //   dd($date);
        $orders = order::where(['status' => 1, 'payment_status' => 'complete'])->whereNull('offer_time_end')->where('date', '=', $date)->get();
        // dd($orders);
                // echo  $orders->schedule;
                // echo date('Y-m-d'); die;
            //     var_dump(explode('-',$order->schedule));
           
            foreach($orders as $order){
         
            $service_details_for_booking = Service::where(['id' =>$order->service_id, 'status' => 1, 'is_service_on' => 1])->first();
            $days=$service_details_for_booking->delivery_days;
            $makesting= "+ $days days";
             
            $time_limit = new DateTime($makesting);
         
                    Order::where('id',$order->id)->update(['offer_time_end'=>$time_limit]);
