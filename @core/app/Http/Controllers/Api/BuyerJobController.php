...
class BuyerJobController extends Controller
{
    ...
    ///cancel order above then 4 hours
    public function AutoCancelOrders()
    {
        ...
        foreach ($orders as $order) {
            ...
            $wallet = Wallet::firstOrCreate(['buyer_id' => $user_id], [
                'balance' => 0,
                'status' => 0,
            ]);
            ...
            if ($order->Custom_offer_id) {
                \App\Models\CustomOffer::where('id', $order->Custom_offer_id)
            ...
            $message_for_seller_admin = __('Buyer cancelled order due to late delivery.') . ' / ' . __('Mnunuzi amekatisha agizo kutokana na ucheleweshaji wa utoaji.');

            try {
                Mail::to($order->email)->send(new OrderMail(strip_tags($mail_subject) . $order->id, $order, $message_for_buyer));
                Mail::to($seller->email)->send(new OrderMail(strip_tags($mail_subject) . $order->id, $order, $message_for_seller_admin));
                Mail::to(get_static_option('site_global_email'))->send(new OrderMail(strip_tags($mail_subject) . $order->id, $order, $message_for_seller_admin));
            } catch (\Exception $e) {
                \Log::error('Mail sending failed for auto-cancel: ' . $e->getMessage());
            }
        }
    ...
    public function cronjobforexpire(){
    $currentDate = Carbon::now();
    BuyerJob::where('promoted', 1)
        ->where('promoteddays', '<=', $currentDate)
        ->update(['promoted' => 0, 'package_id' => 1]);
}

//     public function cronjob(){
//         $currentDate = Carbon::now();
//   BuyerJob::where('promoted', 1)
//             ->whereRaw("DATE_ADD(created_at, INTERVAL promoteddays DAY) <= ?", [$currentDate])
//              ->update(['promoted' => 0,'package_id'=>1]);
