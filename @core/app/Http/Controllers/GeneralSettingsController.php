...
class GeneralSettingsController extends Controller
{
    ...
    public function update_RefferalSetting(Request $request)
    {
        $this->validate($request, [
            ...
            // Business track
            'referral_stage1_business_amount' => 'nullable|numeric|min:0',
            'referral_stage2_business_amount' => 'nullable|numeric|min:0',
            'referral_stage3_business_amount' => 'nullable|numeric|min:0',
            'referral_business_spend_threshold' => 'nullable|numeric|min:0',
            'referral_business_spend_days'    => 'nullable|integer|min:1|max:730',
            // System settings
        ...
        $fields = [
            ...
            'referral_stage1_business_amount',
            'referral_stage2_business_amount',
            'referral_stage3_business_amount',
            'referral_business_spend_threshold',
            'referral_business_spend_days',
      ...
      public function CompanySetting($id=null){
        $Company_request_amount= StaticOption::where(['option_name'=> 'Company_request_amount'])->first();

        return view('backend.general-settings.companySetting',compact('Company_request_amount'));
    }   
    
    
    public function update_CompanySetting(Request $request)
    {
        $all_languages = Language::all();
        foreach ($all_languages as $lang) {
            $this->validate($request, [
                'Company_request_amount' => 'required|nullable|string',
                

            ]);
            $fields = [

                'Company_request_amount',
    ...
    public function adspace($id=null)
    {
        // 
         $adesne_code = StaticOption::where(['option_name'=> 'google_adsense_code'])->first();
          $adsapce_places = Adspace::all();
          if($id != null){
            $single_ad=  Adspace::where(['id'=>$id])->first();
            
          }else{
              
              $single_ad=  Adspace::where(['id'=>'1'])->first();
              
            
          }
         
        return view('backend.general-settings.adspace',compact('adesne_code','adsapce_places','single_ad'));
    }
    public function update_adsspace(Request $request){
        
        $ads_id=$request->parent_id;
        
         $adesne_code = Adspace::where(['id'=>$ads_id])->update(['ads_code_desktop'=>$request->d_google_adsense_code,'ads_code_mobile'=>$request->m_google_adsense_code,'d_width'=>$request->d_width,'d_height'=>$request->d_height,'m_width'=>$request->m_width,'m_height'=>$request->m_height]);
           return redirect()->back()->with(FlashMsg::settings_update());
    }
    public function update_adsensecode(Request $request){
        
         $adesne_code = StaticOption::where(['option_name'=> 'google_adsense_code'])->update(['option_value'=>$request->google_adsense_code]);
           return redirect()->back()->with(FlashMsg::settings_update());
    ...
    public function update_basic_settings(Request $request)
    {
        $this->validate($request, [
            ...
            'site_Challenege_Page' => 'nullable|string',
        ]);
