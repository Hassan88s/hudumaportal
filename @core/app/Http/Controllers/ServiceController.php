...
class ServiceController extends Controller
{
    ...
    public function promotedservices(){
      $brands=  DB::table('services')
->select('*')
->join('feature_service','feature_service.service_id','=','services.id')
->where(['Payment_status' => 'pending'])
->get();
      
         
        
         return view('backend.pages.services.promotedservices',compact('brands'));
    }
    public function change_promoted_status($id=null){
       
       Service::where('id',$id)->update(['featured'=>'1']);
        Featureservice::where('service_id',$id)->update(['Payment_status'=>'complete']);
    ...
    public function change_status($id)
    {
        ...
        if($service_status->status == 1){
            try {
                ...
                $message = str_replace(["@name","@service_id"],[$service_seller_name,$id],$message);
                Mail::to($service_seller_email)->send(new BasicMail([
                    'subject' => __('Service Approve Success'),
                    'message' => $message
                ]));
                
         $seller_id = optional($service->seller)->id;

            notifySeller(
                $seller_id,
                "Huduma Yako Imechapishwa. / Your Service Is Now Live.",
                "a. Hongera! Huduma yako imechapishwa kwa mafanikio",
                [
                    'type' => 'gernalnotifications',
                    'service_id' => $service->id,
                    'id' => uniqid('notif_'),
                    'details' => "Hongera! Huduma yako imechapishwa kwa mafanikio. / Great news! Your service has been successfully posted."
                ]
            );  
