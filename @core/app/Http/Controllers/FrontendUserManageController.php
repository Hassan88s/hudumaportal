...
class FrontendUserManageController extends Controller
{
    ...
    public function userdeleteStatus($id=null,$user_id=null){
        
        //  var_dump($id,$user_id);die;
        
           if($id != NULL){
             
                Accountdeactive::where('id', $id)->update(['status' => 1,'account_status' => 1,]);
               $user_id=  Accountdeactive::where('id', $id)->get();
            //   var_dump( $user_id);die;
                // if(moduleExists('JobPost')) {
                //     BuyerJob::where('buyer_id',$user_id)->update(['status' => 0,'created_at'=> date('Y-m-d H:i:sP')]);
                // }
                toastr_success(__('User Account Delete Successfully'));
            }
