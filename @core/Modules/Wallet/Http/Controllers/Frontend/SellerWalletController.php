...
class SellerWalletController extends Controller
{
    ...
    public function deposit(Request $request)
    {
        ...
        Wallet::where('buyer_id', $buyer_id)->limit(1)->update(['status' => 1]);
        ...
        $description = "Wallet deposit order {$last_deposit_id}";
        ...
        if ($request->selected_payment_gateway === 'clickpesa') {
            try {
                ...
                $result = $service->createCheckoutLink([
                    'orderReference' => \App\Services\ClickPesaService::makeOrderReference('wallet', $last_deposit_id),
                    'amount'         => $total,
                    'currency'       => env('CLICKPESA_CURRENCY', 'TZS'),
                    'description'    => $description,
                    'customerName'   => $name,
                    'customerEmail'  => $email,
                    'callbackUrl'    => route('seller.clickpesa.ipn.wallet'),
                ]);
                ...
                session()->put('history_id', $last_deposit_id);
                session()->put('clickpesa_order_ref', \App\Services\ClickPesaService::makeOrderReference('wallet', $last_deposit_id));
