...
class SellerWalletPaymentController extends Controller
{
    ...
    public function flutterwave_ipn_for_wallet(Request $request)
    {
        ...
        if (isset($payment_data['status']) && $payment_data['status'] === 'complete'){
            ...
            $this->send_jobs_mail($order_id);
            $new_order_id = wrapped_id($order_id);
           toastr_success(__('Your wallet successfully credited'));
            return redirect()->route('seller.wallet.history');
        }
        return $this->cancel_page();
    }
    ...
    /**
     * ClickPesa callback for seller wallet deposit.
     * Called as the customer's browser returns from the hosted checkout.
     * We verify the payment via the API before crediting the wallet.
     */
    public function clickpesa_ipn_for_wallet(Request $request)
    {
        ...
        $status = \App\Services\ClickPesaService::normalizeStatus($payment['status'] ?? null);
        $parsed = \App\Services\ClickPesaService::parseOrderReference($orderRef);
        $deposit_id = $parsed['id'] ?? session()->get('history_id');

        if ($status === 'complete' && $deposit_id) {
            $txn = $payment['id'] ?? $payment['paymentReference'] ?? '';
            $existing = WalletHistory::find($deposit_id);
            if ($existing && $existing->payment_status !== 'complete') {
                $this->update_database($deposit_id, $txn, $deposit_id);
                $this->send_jobs_mail($deposit_id);
            }
            toastr_success(__('Your wallet has been credited successfully.'));
            return redirect()->route('seller.wallet.history');
        }
