...
Route::group(['prefix' => 'admin-home/jobs','as'=>'admin.', 'middleware' => ['globalVariable','setlang']], function () {
    ...
    // jobs Settings
    Route::get( '/settings', 'Backend\JobsController@jobSettings')->name('job.settings');
    Route::post('/job-create-settings-update', 'Backend\JobsController@jobCreateSettingsUpdate')->name('job.create.settings.update');
    Route::get('/job-packages', 'Backend\JobsController@jobsPackages')->name('job.packages');
     Route::post('/job-packages/update', 'Backend\JobsController@update')->name('job_packages.update');
...
Route::group(['prefix'=>'buyer/jobpost','middleware'=>['auth','inactiveuser','UserRoleCheck','userEmailVerify','setlang']],function() {
    ...
    Route::post('/delete/{id}', 'Frontend\JobPostController@job_delete')->name('buyer.job.delete');
        Route::post('/generate-description', 'Frontend\JobPostController@generateDescription');
});
...
Route::group(['prefix' => 'jobpost' ,'middleware' => 'setlang'],function (){
    ...
    Route::get('/flutterwave/ipn','Frontend\BuyerPaymentController@flutterwave_ipn_for_jobs')->name('buyer.flutterwave.ipn.jobs');
      Route::get('/flutterwave/ipn/jobspromoted','Frontend\BuyerPaymentController@flutterwave_ipn_for_jobspromoted')->name('buyer.flutterwave.ipn.jobspromoted');
    ...
    Route::post('/kineticpay-ipn','Frontend\BuyerPaymentController@kineticpay_ipn_for_jobs' )->name('buyer.kineticpay.ipn.jobs');
});
