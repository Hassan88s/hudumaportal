...
class Jobs extends \App\PageBuilder\PageBuilderBase
{
    ...
    {
        return 'home_three/jobs_one.jpg';
    }
    ...
    public function admin_render()
    {
        ...
        $output .= Switcher::get([
            ...
            'info' => __('Area wise Job Filtering Hide/Show')
        ]);
    ...
    public function frontend_render(): string
    {
        ...
        $city_text = $settings['city'] ?? __('Select Region');
        ...
        $job_query = BuyerJob::query()->with(['country','city','area'])->where('status', 1);
        ...
        if(!empty(request()->get('area_id'))){
            $job_query->where('area_id',request()->get('area_id'));
        }
            //
        ...
        //      ->orderByRaw("FIELD(package_id, 3, 2, 1)")
  ...
  $all_jobs = $job_query
    ...
    // IMPORTANT: group the OR conditions so they don't bypass status/is_paid/etc
    ->where(function ($main) {

        $main->where(function ($q) {
            // Non-company: exclude if any hired request exists
            $q->whereHas('buyer', function ($b) {
                $b->where('is_company', '0');
            })
            ->whereDoesntHave('job_request', function ($r) {
                $r->where('is_hired', 1);
            });
        })

        ->orWhere(function ($q) {
            // Company: exclude only if hired requests reached no_of_hiring
            $q->whereHas('buyer', function ($b) {
                $b->where('is_company', '1');
            })
            ->whereRaw("
                (SELECT COUNT(*)
                 FROM job_requests jr
                 WHERE jr.job_post_id = buyer_jobs.id
                   AND jr.is_hired = 1
                ) < buyer_jobs.no_of_hiring
            ");
        });
    ...
    ->paginate($items);
        ...
        if(!empty(request()->get('country_id'))){
            $cities = ServiceCity::select('id', 'service_city')->where('country_id',request()->get('country_id'))->where('status', 1)->get();
                ...
                $areas = [];
        $areas = \App\ServiceArea::select('id', 'service_area')
    ->where('status', 1)
    ->get();
            ...
            foreach ($all_jobs as $job)
            {
               ...
               $buyer_about_route = route('about.buyer.profile',optional($job->buyer)->username);
                ...
                $buyer_name =  optional($job->buyer)->name;
                  $buyer_username = getEnterpriseNamewithoutAuth($job->buyer) ;
                $job_country =  optional($job->country)->country;
                $job_city =  optional($job->city)->service_city;
                $job_area =  optional($job->area)->service_area;
                $promotion_badge = '';
              $promotion_badge = '';
              $Enterprise_badge = '';
              $hiring_note='';
if ($job->promoted) {
    if ($job->package_id == 3) {
        $promotion_badge = '<div class="award_icons" style="display: flex; align-items: center; justify-content: center; font-size: 18px; min-width: 30px; min-height: 30px; background: #fff; color: var(--main-color-one); font-size: 20px; position: absolute; top: 10px; right: 10px; z-index: 9; border-radius: 3px;">
...
if ($job->buyer->is_company == '1') {
        $Enterprise_badge = '  <div class="award_icons">  <span class="badge bg-warning text-dark">Enterprise</span></div>';
    }
                ...
                if($job_country){
                    ...
                    if(!empty($country_on_off)){
                        $job_location_string .= ' '.$job_country;
                    }
                    if(!empty($city_on_off)){
                        $job_location_string .= ' '. $job_area .' '. $job_city;
                    }
                    // if(!empty($city_on_off)){
                    //     $job_location_string .= ' '. $job_area;
                    // }

                    if(!empty($job_location_string)){
                        $job_location = '<span class="single_location" style="color:#fff"><i class="las la-map-marker-alt"></i>' . $job_location_string .'</span>';
                    }
...
<style>
    ...
    .single-service .service-thumb {
        height: 160px !important; /* reduce image height */
    }
    .single-service h5.common-title {
    ...
    <div class="single-service no-margin wow fadeInUp" data-wow-delay=".2s">
        ...
        <div class="services-contents">
            ...
            <div class="service-price">
                ...
                <span class="prices" style="color:#007bff;">{$price}</span>
            </div>
            <hr>
            <ul class="author-tag mt-2">
                <li class="tag-list">
                    <div class="authors d-flex align-items-center">
                        ...
                        <span class="author-title ml-1">{$buyer_username}</span>
                    </div>
                </li>
            </ul>
            <hr>
            <div class="btn-wrapper d-flex flex-wrap mt-2">
               {$apply}
            </div>
        </div>
    </div>
...
JOBS;
        ...
        foreach ($cities as $city) {
            ...
            foreach ($areas as $area) {
                ...
                $area_id = $area->id;
                $selected = !empty(request()->get('area_id')) && request()->get('area_id') == $area->id ? 'selected' : '';
                $area_markup .= <<<AREAS
                    <option {$selected} value="{$area_id}">{$service_area}</option>
                AREAS;
            }  
    ...
    CHILDCATEGORYONOFF;
        ...
        if (!empty($city_on_off)) {
            ...
            <div class="col-lg-3 col-sm-6">
                <div class="single-category-service">
                    <div class="single-select">
                        <select id="search_by_area_job" name="area_id">
