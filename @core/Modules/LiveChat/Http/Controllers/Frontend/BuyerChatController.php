...
class BuyerChatController extends Controller
{
    public function chatUserLogin(Request $request){
        if($request->isMethod('post')){
            ...
            $request->validate([
                ...
                [
                    'user_login_email.required' => sprintf(__('required').'%s',$email_or_username),
    ...
    public function liveChat()
    {
      ...
      $users = LiveChatMessage::select('seller_id')
            ->with('sellerList')
            ->distinct('buyer_id')
    ...
    public function postSendMessage(Request $request)
    {
        ...
        if(!$request->message && $request->image == 'undefined'){
            return ;
        };
        ...
        $receiver_id = $seller_info->id;
        $sender_name =  $seller_info?->name;

        $user_notification_type = 'seller';
        ...
        if($user_details->user_type === 0){
            ...
            $user_notification_type = 'buyer';
            $receiver_id = $request->to_user;
            $sender_name = User::find($request->to_user)?->name;
        }
        ...
        try{
            $response = Http::withToken($pusher_auth)->acceptJson()->post(
                $pusher_auth_url
            ,[
                "interests" => ["debug-".$user_notification_type.$receiver_id, 'message'],
                "fcm" =>[
                    "notification" => [
                        "title" => __("You have received a message from")." ". $sender_name, 
           ...
           $messages = get_static_option('new_message') ?? '';
    ...
    public function chatNameSearch(Request $request)
    {
        $sellers = Order::select('seller_id')
            ...
            ->whereHas('seller', function ($q) use ($request) {
                $q->where('username', 'LIKE', '%' . $request->search_text . '%');
            })
