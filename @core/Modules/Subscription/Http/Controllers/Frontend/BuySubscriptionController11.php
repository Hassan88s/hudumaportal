...
//   var_dump(session()->get('old_data'));die;
        ...
        if(isset($request->subscription_id)){
            ...
            if(!empty($request->apply_coupon_code)){
             ...
             $seller_id_from_seller_subscription_table_for_session = SellerSubscription::select('*')
                        ...
                        ->first();
                        //  dd($seller_id_from_seller_subscription_table_for_session);
                        
    //                     if(!is_null($seller_id_from_seller_subscription_table_for_session)){
    //   TemproryData::insert([
    //       'data'=>json_encode($seller_id_from_seller_subscription_table_for_session),
    ...
    //                     }
                ...
                if(Auth::guard('web')->check()){
                    ...
                    if($seller_id_from_seller_subscription_table){
                        ...
                        // SellerSubscription::where('seller_id',$seller_id_from_seller_subscription_table->seller_id)
                         ...
                         $last_id=TemproryData::insertGetId([
          'data'=>json_encode($data),
          'seller_id'=>Auth::guard('web')->user()->id,
           
          ]);
  
                        $last_subscription_id =  $last_id;
                    }else{
                        $create_subscription = SellerSubscription::create([
                            'subscription_id' => $request->subscription_id,
                            'type' => $request->type,
      ...
      $old_data =TemproryData::where('seller_id',Auth::guard('web')->user()->id)->first();
            if(!is_null($old_data)){
            $request_data=json_decode($old_data->data);
         $total = SellerSubscription::select('total')->where('seller_id',Auth::guard('web')->user()->id)->first();
$date = Carbon::parse($request->expire_date);
$formattedDate = $date->format('Y-m-d H:i:s');
                    $data =    SellerSubscription::where('seller_id',Auth::guard('web')->user()->id)
                            ->update([
                            'subscription_id' => $request_data->subscription_id,
                            'type' => $request_data->type,
                            'initial_service' =>$request_data->initial_service,
                            'initial_job' => $request_data->initial_job,
                            'price' => $request_data->initial_price,
                            'Intial_promoted_services'=>$request_data->Intial_promoted_services,
                            'connect' => ($total->connect+$price),
                            'initial_price' =>($total->total+$request_data->initial_connect),
                            'total' =>$request_data->total,
                            'initial_connect' =>$request_data->initial_connect,
                     ...
                     SubscriptionHistory::where('id', $last_subscription_history)->update([
                        ...
                        Wallet::where('buyer_id',$seller_id)->update([
                            'balance' => $wallet_balance->balance-$request_data->initial_price,
                        ]);
...
//       $seller_id= Auth::guard('web')->user()->id;
//       TemproryData::where('seller_id',Auth::guard('web')->user()->id)->delete();
//      $result= SellerSubscription::create([
//                                 'subscription_id' => $request->subscription_id,
//                                 'type' => $request->type,
//                                 'seller_id'=>Auth::guard('web')->user()->id,
//                                 'initial_service' => $request->initial_service,
