...
public function webhook(Request $request)
  {
    ...
    if ( $request->event == 'charge.completed' &&  $request->data['status'] =='failed') {
       $trx_id= $request->data['tx_ref'];
          $id=   SellerSubscription::where('transaction_id', $trx_id)
                            ->first();
            if(!empty($id)){
        SubscriptionHistory::where('subscription_id',$id->subscription_id)->delete();
        //fetch data
        
            $old_data =TemproryData::where('seller_id',$id->seller_id)->first();
            if(!is_null($old_data)){
            $request=json_decode($old_data->data);
                //update old subscription data
             //update old subscription data
          $create_subscription = SellerSubscription::create([
                            'subscription_id' => $request->subscription_id,
                            'type' => $request->type,
                            'initial_service' =>$request->initial_service,
                            'initial_job' => $request->initial_job,
                            'price' => $request->price,
                            'Intial_promoted_services'=>$request->Intial_promoted_services,
                            'connect' => 0,
                            'initial_price' =>$request->initial_price,
                            'total' =>$request->total,
                            'initial_connect' =>$request->initial_connect,
        ...
        TemproryData::where('seller_id',$id->seller_id)->delete();
        $id->delete();
            }else{
      
        $trx_id= $request->data['tx_ref'];
        $data= Featureservice::where('transaction_id', $trx_id)->delete();       
   \Log::info( $data);
  ...
  public function showwebhook(){
       $old_data =TemproryData::where('seller_id',Auth::guard('web')->user()->id)->first();
       
       var_dump(json_decode($old_data->data));
    ...
    public function subscription_payment_cancel_static()
    {
        ...
        if(session()->get('old_data') != NULL ){
      
       $request= session()->get('old_data');
       $seller_id= session()->get('order_id');
      
     TemproryData::where('seller_id',Auth::guard('web')->user()->id)->delete();
         SellerSubscription::where('id',$seller_id)
                            ->update([
                                'subscription_id' => $request->subscription_id,
                                'type' => $request->type,
                                'initial_service' => $request->initial_service,
        ...
        }elseif(session()->get('order_id') != NULL){
             $seller_id= session()->get('order_id');
             SellerSubscription::where('id',$seller_id)
                            ->delete();
                            TemproryData::where('seller_id',Auth::guard('web')->user()->id)->delete();
    ...
    public function buy_subscription(Request $request)
    {
        ...
        if(isset($request->subscription_id)){
            ...
            if ($request->price == 0){
                ...
                if(!empty($check_already_has_free_subscription_in_this_month)){
            ...
            if($request->type=='monthly' && $request->month=='1'){
                $expire_date = Carbon::now()->addDays(30);
                $connect = $request->connect;
                $service = $request->service;
                $job = $request->job;
                $pro_job = $request->projob;
                $projects_allowed = $request->projects_allowed;
                $cashback_percentage = $request->cashback_percentage;
            }elseif($request->type=='monthly' && $request->month=='3'){
                $expire_date = Carbon::now()->addDays(90);
                $connect = $request->connect*$request->month;
                $service = $request->service*$request->month;
                $job = $request->job*$request->month;
                $pro_job = $request->projob*$request->month;
                $projects_allowed = $request->projects_allowed*$request->month;
                $cashback_percentage = $request->cashback_percentage*$request->month;  
            }
            elseif($request->type=='monthly' && $request->month=='6'){
            ...
            elseif($request->type=='yearly'){
                ...
                $job = 1000000;
                $pro_job = 1000000;
                $projects_allowed = 1000000;
                $cashback_percentage = 1000000; 
            }
            $price = $request->price;
            ...
            if(!empty($request->apply_coupon_code)){
             ...
             $seller_id_from_seller_subscription_table_for_session = SellerSubscription::select('*')
                        ->where('seller_id',$user_id)
                        ->first();
    
             session()->put('old_data',$seller_id_from_seller_subscription_table_for_session);
            //  start data
            if($request->seller_payment_later != 'later'){
                if(Auth::guard('web')->check()){

                    $user_name = Auth::guard('web')->user()->username;
                    $user_email = Auth::guard('web')->user()->email;
                    $user_id = Auth::guard('web')->user()->id;
                    $seller_id_from_seller_subscription_table = SellerSubscription::select('id','seller_id','connect','expire_date')
                        ->where('seller_id',$user_id)
                        ->first();
                    $trxno= 'flw_' . uniqid(time());
                    
                    // Store the subscription details in session (instead of database) until payment is confirmed
                $request->session()->put('subscription_data', [
                    ...
                    'service' => $service,
                    'job' => $job,
                    'Intial_promoted_services'=> $pro_job,
                    'connect' => $connect,
                    ...
                    'payment_gateway' => $request->selected_payment_gateway,
                    'payment_status' => $payment_status, // Pending until payment is confirmed
                    
                    'initialprojects_allowed' => $projects_allowed,
                    'initialcashback_percentage' => $cashback_percentage,
                    'sms_notifications' =>  $request->sms_notifications,
                    'Website_enabled' => $request->Website_enabled,
                    'personal_enabled' => $request->personal_enabled,
                    'partialpayment_enabled' => $request->partialpayment_enabled,
                    'aidescription_enabled' => $request->aidescription_enabled,
                    ...
                    ///for wallet
                if ($request->selected_payment_gateway === null) {
                    ...
                    if ($price > 0) {
                        ...
                        if (empty($wallet_balance) || $wallet_balance->balance < $price) {
                            $shortage_balance = $price - ($wallet_balance->balance ?? 0);
                    ...
                    $last_subscription_history = $create_history->id;
                            if($seller_id_from_seller_subscription_table){
                                // Delete existing seller subscription
                                SellerSubscription::where('seller_id', $seller_id_from_seller_subscription_table->seller_id)->delete();
                                ...
                                // Same as before (create new if none exists)
                                $create_subscription = SellerSubscription::create([
                                    'subscription_id' => $request->subscription_id,
                                    'type' => $request->type,
                                    'initial_service' => $service,
                            ...
                            if($wallet_balance->balance >=  $price){
                        ...
                        Wallet::where('buyer_id',$seller_id)->update([
                         ...
                         WalletHistory::create([
                            ...
                            'amount' => $subscription_details->initial_price,
                            'payment_gateway' => 'Pay for Subscription',
                            'payment_status' => 'complete',
                            'status' => 1,
                    ...
                    try {
                      ...
                      $type = str_replace("@type", $request->type, "@type");
                        $price = str_replace("@price", float_amount_with_currency_symbol($request->price), "@price");
                        $connect = str_replace("@connect", $request->connect, "@connect");
                        
                        // $message = __('Hello,') . "\n\n" . 
                        //           __('You have successfully bought a subscription.') . "\n\n" . 
                        //           __('Your subscription info:') . "\n" . 
                        //           __('Subscription Type:') . ' ' . $type . "\n" . 
                        //           __('Subscription Price:') . ' ' . $price . "\n" . 
                                  
                        //           "-----------------------\n\n" . 
                        //           "Habari,\n\n" . 
                        //           "Umefanikiwa kununua usajili.\n\n" . 
                        //           "Maelezo ya usajili wako:\n" . 
                        //           "Aina ya Usajili: " . $type . "\n" . 
                        //           "Bei ya Usajili: " . $price . "\n" . 
                        //           "Muunganisho wa Usajili: " . $connect;


                        // $message = str_replace(["@type","@price"],[$request->type,float_amount_with_currency_symbol($price)],$message);
                        // Mail::to($user_email)->send(new BasicMail([
                        //     'subject' =>get_static_option('buy_subscription_email_subject') ?? __('New Subscription') . ' / Usajili Mpya',
                        //     'message' => $message
                        // ]));

                        $message = get_static_option('buy_subscription_admin_message') ?? '';
                        $message = str_replace(["@type","@price","@seller_name","@seller_email"],[$request->type,float_amount_with_currency_symbol($price),$user_name,$user_email],$message);
                        ...
                        $connect = $request->type =='lifetime' ? __("No Limit") : $request->connect;
                        $message = get_static_option('buy_subscription_seller_message') ?? '';
                        $message = str_replace(["@type","@price","@expiredate","@seller_name","@seller_email"],[$request->type,float_amount_with_currency_symbol($price),$expire_date,$user_name,$user_email],$message);
                        Mail::to($user_email)->send(new BasicMail([
                            'subject' =>get_static_option('buy_subscription_email_subject') ?? __('New Subscription'),
                            'message' => $message
                        ]));
                    ...
                    notifySeller(
                    $seller_id,
                    "Umejiunga na uanachama wa $request->type .Furahia faida maalum na zana za kukuza huduma zako. / You’ve subscribed to the $request->type  membership.Enjoy exclusive benefits and tools to grow your freelance career.", //p
                    "Umejiunga na uanachama wa $request->type .Furahia faida maalum na zana za kukuza huduma zako.", //sms
                    [
                        'type' => 'gernalnotifications',
                        'id' => uniqid('notif_'),
                        'details' => "Umejiunga na uanachama wa  $request->type .Furahia faida maalum na zana za kukuza huduma zako. / You’ve subscribed to the  $request->type   membership.Enjoy exclusive benefits and tools to grow your freelance career." //p
                    ]
                );     
                      ...
                      SellerSubscription::where('id',  $last_subscription_id)->delete();
                        $shortage_balance =  $price-$wallet_balance->balance;
                        toastr_warning('Your wallet has '.float_amount_with_currency_symbol($shortage_balance).' shortage to order this service. Please Credit your wallet first and try again.');
                        return back();
                    }
                   }else{
                         toastr_warning('Your wallet has have no amount. Please Credit your wallet first and try again .');
                        return back();
                   }
            ...
            if( $price == 0 && $request->selected_payment_gateway === 'flutterwave'){
                ...
                // return back();
                 ...
                 $random_order_id_1 = Str::random(30);
                    ...
                    try {
                        $connect = $request->type =='lifetime' ? __("No Limit") : $request->connect;
                        $message = get_static_option('buy_subscription_seller_message') ?? '';
                        $message = str_replace(["@type","@price","@connect","@expiredate","@seller_name","@seller_email"],[$request->type,float_amount_with_currency_symbol($price),$connect,$expire_date,$user_name,$user_email],$message);
                        Mail::to($user_email)->send(new BasicMail([
                            'subject' =>get_static_option('buy_subscription_email_subject') ?? __('New Subscription'),
                            'message' => $message
                        ]));
...
///end wallet
            ...
            if ($request->selected_payment_gateway === 'clickpesa') {
                try {
                    // Create a real SellerSubscription row FIRST so we have a stable id
                    // for the ClickPesa order reference. Any existing pending row for
                    // this seller is replaced (they cancelled or retried).
                    $sellerId = Auth::guard('web')->user()->id;

                    // Clean up any previous pending row for this seller so we don't leak
                    SellerSubscription::where('seller_id', $sellerId)
                    ...
                    $create_subscription = SellerSubscription::create([
                        ...
                        'payment_status'             => 'pending',
                        'initialprojects_allowed'    => $projects_allowed ?? 0,
                        'initialcashback_percentage' => $cashback_percentage ?? 0,
                        'sms_notifications'          => $request->sms_notifications,
                        'Website_enabled'            => $request->Website_enabled,
                        'personal_enabled'           => $request->personal_enabled,
                        'partialpayment_enabled'     => $request->partialpayment_enabled,
                    ...
                    // Now build the ClickPesa checkout for the real SellerSubscription id
                    $service_cp = new \App\Services\ClickPesaService();
                    $orderRef = \App\Services\ClickPesaService::makeOrderReference('subscription', $last_subscription_id);
                    $result = $service_cp->createCheckoutLink([
                        'orderReference' => $orderRef,
                        'amount'         => $price,
                        'currency'       => env('CLICKPESA_CURRENCY', 'TZS'),
                        'description'    => __('Subscription').' #'.$last_subscription_id,
                        'customerName'   => $user_name,
                        'customerEmail'  => $user_email,
                        'callbackUrl'    => route('clickpesa.return'),
                    ]);
                    if (!$result['ok']) {
                        // Roll back the pending records we just created
                        SellerSubscription::where('id', $last_subscription_id)->delete();
                        SubscriptionHistory::where('id', $create_history->id)->delete();
                        \Toastr::error('ClickPesa: '.($result['error'] ?? 'unknown error'));
                        return redirect()->back();
                    }
                    session()->put('history_id', $create_history->id);
            ...
            if($request->selected_payment_gateway === 'manual_payment') {
                ...
                elseif($request->selected_payment_gateway === 'flutterwave'){
                    ...
                    $redirect_url = $flutterwave->charge_customer([
                        ...
                        'order_id' => $last_subscription_id,
                        'track' => \Str::random(36),
                        'cancel_url' => route(self::CANCEL_ROUTE,$trxno),
                        'success_url' => route(self::SUCCESS_ROUTE,$trxno),
                        'email' => $user_email,
                        'name' => $user_name,
                        'payment_type' => 'order',
                        'tx_ref'=> $trxno,
